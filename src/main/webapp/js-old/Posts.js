...
var Posts = function(context){
    ...
    var clickPlayer = new Audio();
    clickPlayer.src =  _context + "/media/audio/click.wav";


    var phrases = [
        "It's time for a cool change.",
        "Let your love shine, and you'll know what I mean, that is the reason!"
    ]
    ...
    function updateNotificationsDiv(request){
        ...
        if(data.length > 0){
            ...
            renderMustache(notificationsTemplate, notificationsContainer, data).then(function(){
                ...
                addEventListenersInvites();
            });
        }
        else{
            notificationsContainer.innerHTML = '<p style="background:#d953a0;font-size:17px;">No new notifications</p>'
            notificationsOuterDiv.setAttribute("data-opened", "true")
            notificationsOuterDiv.style.display = "block"
        }
    }

    function addEventListenersInvites(){
        var inviteRefs = document.getElementsByClassName("invite-ref")
        var inviteRefsArr = Array.from(inviteRefs)

        inviteRefsArr.forEach(function(ref, index){
            ref.addEventListener("click", function(event){
                friendInvites.dispatchTransitionInvitesPageEvent(event)
            });
        })
    }
    ...
    function renderPosts(request){
        ...
        sessionStorage.setItem("amadeus-current", "posts")
    }


    function addEditImageEvents(){
        var imageInputs = document.getElementsByClassName("edit-image-input")
        var imageInputsArr = Array.from(imageInputs)

        imageInputsArr.forEach(function(imageInput, index){
            imageInput.removeEventListener("change", function(event){
                var id = imageInput.getAttribute("data-id")
                var imageButton = document.getElementById("edit-image-button-" + id)
                imageButton.removeEventListener("click", function(event){})
            })

            imageInput.addEventListener("change", function(event){
                var id = imageInput.getAttribute("data-id")
                var imageForm = document.getElementById("edit-image-form-" + id)
                var uri = _context + "/post/image/add/" + id;
                web.publish(uri, imageForm).then(retrievePosts).then(clearForm)
            })
        })
    }


    function addDeleteImageEvents() {
        var deleteInputs = document.getElementsByClassName("delete-image")
        var deleteInputsArr = Array.from(deleteInputs)
        deleteInputsArr.forEach(function(deleteInput, index) {
            deleteInput.addEventListener("click", function(event){
                var id = deleteInput.getAttribute("data-id")
                var imageUri = deleteInput.getAttribute("data-uri")

                var deleteForm = document.getElementById("delete-image-form")
                var deleteImageInput = document.getElementById("delete-image-input")

                deleteImageInput.value = imageUri

                var uri = _context + "/image/delete/" + id
                web.publish(uri, deleteForm).then(retrievePosts).then(function(){
    ...
    var editText = document.getElementById("edit-post-text")

    function subscriber (mutations) {
        mutations.forEach((mutation) => {

            var id = mutation.target.dataset.id
            var $content = _.jQuery("#post-content-" + id)

            $content.children().each(function(index, element){
                _.jQuery(element).contents().unwrap();
            })

            editText.value = mutation.target.innerHTML
        });
    }

    function addEditObservers(){
        var editableDivs = document.getElementsByClassName("post-comment")
        var editableDivsArr = Array.from(editableDivs)

        editableDivsArr.forEach(function(postDiv, index) {
            const observer = new MutationObserver(subscriber);
            observer.observe(postDiv, config);

            postDiv.addEventListener("keyup", function(event){
                var keycode = (event.keyCode ? event.keyCode : event.which);
                var $current = _.jQuery(event.target)
            ...
            postDiv.addEventListener("focusin", function(){
                postDiv.style.backgroundColor = "#fcffee"
            })

            postDiv.addEventListener("focusout", function(){
                postDiv.style.backgroundColor = "#fff"
            })

            // var $editables = _.jQuery(".edit-fade-in")
            // $postDiv.hover(function(){
            //     $editables.fadeIn(100)
            // }, function(){
            //     $editables.fadeOut(102)
            // })
        })

        var updateButtons = document.getElementsByClassName("edit-update-button")
        var updateButtonsArr = Array.from(updateButtons)

        updateButtonsArr.forEach(function(updateButton, index) {
            updateButton.addEventListener("click", function(){
                var id = updateButton.getAttribute("data-id")
                var postDiv = document.getElementById("post-content-" + id)
                editText.value = postDiv.innerHTML
                performUpdate(id)
            })
        })
    }
    ...
    function performUpdate(id){
        var uri = _context + "/posts/update/" + id
        web.publish(uri, editForm).then(retrievePosts).catch(error)
    }
