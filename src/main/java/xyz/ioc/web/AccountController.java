...
//				parakeet.store("imageUri", sessionAccount.getImageUri());
	...
	private List<Notification> getNotifications(Account account){
		List<xyz.ioc.model.Notification> notifications = notificationDao.notifications(account.getId());
		for(xyz.ioc.model.Notification notification: notifications){
			Account a = accountDao.get(notification.getAuthenticatedAccountId());
			notification.setName(a.getName());
		}

		Comparator<xyz.ioc.model.Notification> comparator = new Comparator<xyz.ioc.model.Notification>() {
			@Override
